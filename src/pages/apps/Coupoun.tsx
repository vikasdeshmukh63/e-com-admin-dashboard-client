...
const Coupoun = () => {
  ...
  const [coupoun, setCoupoun] = useState<string>("");

  const copyText = async (coupoun: string) => {
    await window.navigator.clipboard.writeText(coupoun);
    setIsCopied(true);
  };
  ...
  return (
    <div className="adminContainer">
      ...
      <main className="dashboardAppContainer">
        <h1>Coupon</h1>
        <section>
          <form className="coupounForm" onSubmit={submitHandler}>
          ...
          {coupoun && (
            <code>
              {coupoun} <span onClick={() => copyText(coupoun)}>{isCopied ? "Copied" : "Copy"}</span>
            </code>
          )}
        </section>
      </main>
